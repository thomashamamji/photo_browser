...
router.get("/album/list", (req, res, next) => {
    ...
    next();
}, passport.authenticate('jwt', {session : false}), index.list_albums);
router.get("/album/add/:name", passport.authenticate('jwt', {session : false}), index.add_album);
router.post("/album/media/add", (req, res, next) => {
    console.log(req.headers);
    next();
}, passport.authenticate('jwt', {session : false}), index.add_media);
